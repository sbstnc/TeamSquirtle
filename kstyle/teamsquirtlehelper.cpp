...
#if TEAMSQUIRTLE_HAVE_X11 && QT_VERSION < 0x050000
#include <X11/Xlib-xcb.h>
 ...
 namespace TeamSquirtle
 {
    ...
    QColor Helper::buttonOutlineColor( const QPalette& palette, bool mouseOver, bool hasFocus, qreal opacity, AnimationMode mode ) const
    {
        ...
        QColor outline( KColorUtils::mix( palette.color( QPalette::Button ), palette.color( QPalette::ButtonText ), 0.2 ) );
        if( mode == AnimationHover )
        {
        ...
        bool hasFocus, bool sunken ) const
    {
        ...
        if( outline.isValid() )
        {
            ...
            painter->setPen( QPen( QBrush( outline.lighter( hasFocus ? 103:100 ) ), 1.0 ) );
        ...
        if( color.isValid() )
        {
            ...
            painter->setBrush( color.lighter( hasFocus ? 103:100 ) );
    ...
    void Helper::renderTabWidgetFrame(
        QPainter* painter, const QRect& rect,
        const QColor& color, const QColor& outline, Corners corners, bool roundCorners ) const
    {
        ...
        // render
        QPainterPath path;
        if( roundCorners ) path = roundedPath( frameRect, corners, radius );
        else path = roundedPath( frameRect, corners, 0.0 );
        painter->drawPath( path );
    ...
    void Helper::renderArrow( QPainter* painter, const QRect& rect, const QColor& color, ArrowOrientation orientation ) const
    {
        ...
        switch( orientation )
        {
            case ArrowUp: arrow << QPointF( -2, 2 ) << QPointF( 0, 0 ) << QPointF( 2, 2 ); break;
            case ArrowDown: arrow << QPointF( -2, -2 ) << QPointF( 0, 0 ) << QPointF( 2, -2 ); break;
            case ArrowLeft: arrow << QPointF( 2, -2 ) << QPointF( 0, 0 ) << QPointF( 2, 2 ); break;
            case ArrowRight: arrow << QPointF( -2, -2 ) << QPointF( 0, 0 ) << QPointF( -2, 2 ); break;
            default: break;
        }
